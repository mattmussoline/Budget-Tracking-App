...
type RoadmapDashboardProps = {
  ...
  pageTitle?: string;
  /** Optional extra line under the roadmap's own "N titles · range · minutes" summary. */
  pageDescription?: string;
  /** Licensing Summary for this fiscal year, linked under the breakdown. */
  summaryHref?: string;
  fiscalYearId: string;
...
export function RoadmapDashboard({ pageTitle = "Roadmap", pageDescription, summaryHref, fiscalYearId, roadmapItems, ongoingSeries, categories, startMonth, fiscalYearStartMonth = getFiscalYearStartMonthForMonth(startMonth), monthCount, routeBasePath = "/roadmap", fiscalYearLabel, isDemo }: RoadmapDashboardProps) {
  ...
  const mixLists = buildRoadmapMix(filteredItems, mixFilter);
  ...
  /** Split the live backlog the way the board reads it: no date at all, versus dated outside the window. */
  const undatedBacklog = otherBacklog.filter((item) => !getRoadmapMonthKey(item.releaseDate));
  const outsideWindowBacklog = otherBacklog.filter((item) => Boolean(getRoadmapMonthKey(item.releaseDate)));
  ...
  const totalSecuredMinutes = minutesByBudgetSource.reduce((sum, line) => sum + line.minutes, 0);
  const windowRangeLabel = `${formatShortMonth(months[0].date)} – ${formatShortMonth(months[months.length - 1].date)}`;
  const summaryLine = filterSummary
  ...
  return <div className={cn("grid min-w-0", !railCollapsed && "xl:grid-cols-[288px_minmax(0,1fr)] xl:items-start")}>
    ...
    <div className="grid min-w-0 gap-6 px-6 pb-14 pt-7 md:px-8">
      <div className="flex flex-wrap items-end justify-between gap-x-8 gap-y-4">
        <div className="grid min-w-0 gap-2">
          <h1 className="font-display text-[40px] leading-[1.1] md:text-[46px]">{pageTitle}</h1>
          <p className="text-[15px] leading-relaxed text-muted [text-wrap:pretty]">{summaryLine}</p>
          {pageDescription ? <p className="max-w-3xl text-sm leading-relaxed text-muted [text-wrap:pretty]">{pageDescription}</p> : null}
        </div>
        <nav aria-label="Roadmap timeline controls" className="flex flex-wrap items-center gap-3">
          {focusedMonthKey ? <SoftButton type="button" variant="secondary" className="rounded-none" onClick={() => setFocusedMonthKey(null)}><ChevronLeft className="h-4 w-4" aria-hidden="true" />Show all months</SoftButton> : null}
          {viewMode === "cards" ? <>
            <div className="flex items-center overflow-hidden border border-hairline bg-panel">
              <Link aria-label="Previous" className="inline-flex min-h-10 items-center px-3 py-1.5 text-muted transition-colors hover:bg-panel-warm hover:text-foreground" href={href(shiftMonthAnchor(startMonth, -monthCount))}><ChevronLeft className="h-4 w-4" aria-hidden="true" /></Link>
              <span aria-hidden="true" className="w-px self-stretch bg-hairline" />
              <Link className="inline-flex min-h-10 items-center bg-formed-blue-soft px-4 py-1.5 text-[13.5px] font-semibold text-formed-blue" href={href(today)}>Today</Link>
              <span aria-hidden="true" className="w-px self-stretch bg-hairline" />
              <Link aria-label="Next" className="inline-flex min-h-10 items-center px-3 py-1.5 text-muted transition-colors hover:bg-panel-warm hover:text-foreground" href={href(shiftMonthAnchor(startMonth, monthCount))}><ChevronRight className="h-4 w-4" aria-hidden="true" /></Link>
            </div>
            <div className="flex items-center overflow-hidden border border-hairline bg-panel">
              {([6, 9, 12] as const).map((count) => <Link key={count} href={href(startMonth, count)} aria-label={`${count} months`} aria-current={monthCount === count ? "page" : undefined} className={cn("inline-flex min-h-10 items-center px-4 py-1.5 text-[13.5px] transition-colors", monthCount === count ? "bg-augustine-blue font-semibold text-white" : "font-semibold text-muted hover:bg-panel-warm hover:text-foreground")}>{count === 12 ? "12 months" : count}</Link>)}
            </div>
          </> : <div className="flex items-center overflow-hidden border border-hairline bg-panel">
            <button type="button" aria-label="Previous month" onClick={() => setCalendarMonthKey((key) => shiftMonthAnchor(key, -1))} className="inline-flex min-h-10 items-center px-3 py-1.5 text-muted transition-colors hover:bg-panel-warm hover:text-foreground"><ChevronLeft className="h-4 w-4" aria-hidden="true" /></button>
            <span aria-hidden="true" className="w-px self-stretch bg-hairline" />
            <button type="button" onClick={() => setCalendarMonthKey(parseMonthAnchor(null))} className="inline-flex min-h-10 items-center bg-formed-blue-soft px-4 py-1.5 text-[13.5px] font-semibold text-formed-blue">Today</button>
            <span aria-hidden="true" className="w-px self-stretch bg-hairline" />
            <button type="button" aria-label="Next month" onClick={() => setCalendarMonthKey((key) => shiftMonthAnchor(key, 1))} className="inline-flex min-h-10 items-center px-3 py-1.5 text-muted transition-colors hover:bg-panel-warm hover:text-foreground"><ChevronRight className="h-4 w-4" aria-hidden="true" /></button>
          </div>}
          <div className="flex items-center overflow-hidden border border-hairline bg-panel" role="group" aria-label="Roadmap view">
            <button type="button" aria-pressed={viewMode === "cards"} onClick={() => setViewMode("cards")} className={cn("inline-flex min-h-10 items-center px-4 py-1.5 text-[13.5px] transition-colors", viewMode === "cards" ? "bg-augustine-blue font-semibold text-white" : "font-semibold text-muted hover:bg-panel-warm hover:text-foreground")}>Cards</button>
            <span aria-hidden="true" className="w-px self-stretch bg-hairline" />
            <button type="button" aria-pressed={viewMode === "calendar"} onClick={() => setViewMode("calendar")} className={cn("inline-flex min-h-10 items-center px-4 py-1.5 text-[13.5px] transition-colors", viewMode === "calendar" ? "bg-augustine-blue font-semibold text-white" : "font-semibold text-muted hover:bg-panel-warm hover:text-foreground")}>Calendar</button>
          </div>
        </nav>
      </div>

      {viewMode === "cards" ? <section className="min-w-0">
        {/* One fixed-height viewport: months scroll sideways, a long month scrolls down inside it. */}
        <div data-testid="roadmap-month-scroll" className="roadmap-scroll flex h-[440px] gap-4 overflow-auto">
          {displayedMonths.map((month) => {
            ...
            return <article data-testid="roadmap-month-column" key={month.key} className={cn("shrink-0 self-start overflow-hidden border border-hairline bg-panel", focusedMonthKey ? "w-full min-w-full" : "w-[300px]")}>
              <button type="button" aria-label={`Focus ${month.label}`} onClick={() => setFocusedMonthKey(month.key)} className="flex w-full flex-wrap items-baseline justify-between gap-2 border-b border-hairline bg-panel-warm px-4 py-3.5 text-left transition-colors hover:bg-hairline/40">
                <h3 className="font-display text-[22px] leading-tight">{month.label}</h3>
                <span className="text-[11.5px] font-semibold text-faint">{items.length} {items.length === 1 ? "release" : "releases"} · {monthMinutes} min</span>
              </button>
              <div className={cn("grid", focusedMonthKey && "md:grid-cols-2 xl:grid-cols-3")}>
                {items.length ? items.map((item) => <RoadmapCard key={item.id} variant="month" item={item} category={item.categoryId ? categoryMap.get(item.categoryId) : undefined} categories={categories} fiscalYearId={fiscalYearId} isDemo={isDemo} providerOptions={providerOptions} isOpen={activeRoadmapItemId === item.id} onOpen={() => setActiveRoadmapItemId(item.id)} onClose={() => setActiveRoadmapItemId((currentId) => currentId === item.id ? null : currentId)} />) : <p className="border-b border-hairline px-4 py-5 text-xs text-faint">Nothing scheduled this month.</p>}
              </div>
              <div className="flex flex-wrap items-center justify-between gap-2 px-4 py-3">
                <AddRoadmapModal triggerLabel="Add" triggerAriaLabel={`Add item to ${month.label}`} triggerIcon={<Plus className="h-3 w-3" aria-hidden="true" />} triggerClassName="min-h-0 border-0 bg-transparent p-0 text-xs !text-muted hover:!bg-transparent hover:!text-foreground">
      ...
      <div className="grid gap-5 border-t border-hairline pt-6 xl:grid-cols-[minmax(0,1.6fr)_minmax(0,1fr)]">
        <SeriesTable fiscalYearId={fiscalYearId} ongoingSeries={ongoingSeries} isDemo={isDemo} />
        <details id="roadmap-backlog" data-testid="roadmap-backlog" className="group self-start border border-hairline bg-panel-warm" open>
          <summary className="flex cursor-pointer list-none items-start justify-between gap-3 px-5 py-4 [&::-webkit-details-marker]:hidden">
            <div className="grid min-w-0 gap-1">
              <h2 className="font-display text-[21px] leading-tight">Backlog</h2>
              <p className="text-xs text-muted [text-wrap:pretty]">Undated items and anything outside this window.</p>
            </div>
            <span className="flex shrink-0 items-center gap-2">
              <span className="grid h-6 min-w-6 place-items-center bg-panel px-1.5 text-[11px] font-semibold text-muted ring-1 ring-hairline">{backlog.length}</span>
              <span className="grid h-7 w-7 shrink-0 place-items-center border border-hairline bg-panel text-muted">
                <Plus className="h-3.5 w-3.5 group-open:hidden" aria-hidden="true" />
                <Minus className="hidden h-3.5 w-3.5 group-open:block" aria-hidden="true" />
                <span className="sr-only">Expand Backlog section</span>
              </span>
            </span>
          </summary>
          {/* The height lives on the body, not the <details>, so collapsing still shrinks the panel. */}
          <div className="roadmap-scroll grid max-h-[336px] content-start gap-4 overflow-y-auto border-t border-hairline px-5 pb-4 pt-4">{backlog.length ? <>
            {undatedBacklog.length ? <BacklogGroup title="Undated — needs a release date" count={undatedBacklog.length} testId="backlog-other-content" defaultOpen>{undatedBacklog.map((item) => <RoadmapCard key={item.id} item={item} category={item.categoryId ? categoryMap.get(item.categoryId) : undefined} categories={categories} fiscalYearId={fiscalYearId} isDemo={isDemo} providerOptions={providerOptions} isOpen={activeRoadmapItemId === item.id} onOpen={() => setActiveRoadmapItemId(item.id)} onClose={() => setActiveRoadmapItemId((currentId) => currentId === item.id ? null : currentId)} />)}</BacklogGroup> : null}
            {outsideWindowBacklog.length ? <BacklogGroup title="Outside this window" count={outsideWindowBacklog.length} testId="backlog-outside-window" defaultOpen>{outsideWindowBacklog.map((item) => <RoadmapCard key={item.id} item={item} category={item.categoryId ? categoryMap.get(item.categoryId) : undefined} categories={categories} fiscalYearId={fiscalYearId} isDemo={isDemo} providerOptions={providerOptions} isOpen={activeRoadmapItemId === item.id} onOpen={() => setActiveRoadmapItemId(item.id)} onClose={() => setActiveRoadmapItemId((currentId) => currentId === item.id ? null : currentId)} />)}</BacklogGroup> : null}
            {releasedBacklog.length ? <BacklogGroup title="Already released content" count={releasedBacklog.length} testId="backlog-released-content">{releasedByMonth.map(({ monthKey, monthLabel, items }) => <BacklogGroup key={monthKey} title={monthLabel} count={items.length} testId={`released-month-${monthKey}`}>{items.map((item) => <RoadmapCard key={item.id} item={item} category={item.categoryId ? categoryMap.get(item.categoryId) : undefined} categories={categories} fiscalYearId={fiscalYearId} isDemo={isDemo} providerOptions={providerOptions} isOpen={activeRoadmapItemId === item.id} onOpen={() => setActiveRoadmapItemId(item.id)} onClose={() => setActiveRoadmapItemId((currentId) => currentId === item.id ? null : currentId)} />)}</BacklogGroup>)}</BacklogGroup> : null}
          </> : <p className="text-sm text-faint">No backlog items.</p>}</div>
        </details>
      </div>

      <RoadmapMix lists={mixLists} rankedTitleCount={filteredItems.length} isFiltered={Boolean(filterSummary)} activeFilter={mixFilter} onPick={pickMixFilter} summaryHref={summaryHref} summaryLabel={fiscalYearLabel} />
    </div>
...
function BacklogGroup({ title, count, testId, children, defaultOpen = false }: { title: string; count: number; testId: string; children: ReactNode; defaultOpen?: boolean }) {
  return <details data-testid={testId} className="grid gap-3" open={defaultOpen}>
    <summary className="flex cursor-pointer list-none items-center justify-between gap-3 text-[13px] font-semibold [&::-webkit-details-marker]:hidden">
      <span>{title}</span>
      <span className="grid h-5 min-w-5 place-items-center bg-panel px-1.5 text-[10.5px] font-semibold text-muted ring-1 ring-hairline">{count}</span>
    </summary>
    <div className="grid gap-2.5">{count ? children : <p className="bg-panel p-3 text-sm font-semibold text-muted">No items.</p>}</div>
  </details>;
}
...
function RoadmapCalendar({ monthKey, items, categoryMap, categories, fiscalYearId, providerOptions, isDemo, activeRoadmapItemId, setActiveRoadmapItemId }: RoadmapCalendarProps) {
  ...
  return <section className="min-w-0 border border-hairline bg-panel-warm p-4">
    <div className="mb-3 flex flex-wrap items-end justify-between gap-3">
      <div className="grid gap-0.5">
        <h2 className="font-display text-[26px] leading-tight">{monthLabel}</h2>
...
function RoadmapCalendarDay({ day, isToday, items, categoryMap, categories, fiscalYearId, providerOptions, isDemo, activeRoadmapItemId, setActiveRoadmapItemId }: { day: number; isToday: boolean } & Omit<RoadmapCalendarProps, "monthKey" | "items"> & { items: RoadmapItem[] }) {
  ...
  return <div className={cn("flex min-h-28 flex-col gap-1 border border-hairline bg-panel p-1.5", isToday && "border-formed-blue ring-1 ring-formed-blue")}>
...
function SeriesTable({ fiscalYearId, ongoingSeries, isDemo }: { fiscalYearId: string; ongoingSeries: OngoingSeries[]; isDemo?: boolean }) {
  ...
  return (
    <section className="grid h-[420px] grid-rows-[auto_auto_minmax(0,1fr)] overflow-hidden border border-hairline bg-panel-warm">
      <div className="flex flex-wrap items-baseline justify-between gap-3 px-5 py-4">
        <h2 className="font-display text-[21px] leading-tight">Ongoing series cadence</h2>
        <button type="button" onClick={() => setIsAdding((value) => !value)} className="text-[13px] font-semibold text-formed-blue transition-colors hover:text-formed-blue-hover">
          {isAdding ? "Close" : "+ Add series"}
        </button>
      </div>
      <div className="grid grid-cols-[1fr_0.6fr_0.9fr_0.5fr_auto] gap-4 border-y border-hairline bg-panel px-5 py-3 text-[11px] font-semibold uppercase tracking-[0.06em] text-muted">
        <span>Series</span>
        <span>Cadence</span>
      ...
      <div className="roadmap-scroll min-h-0 overflow-y-auto bg-panel">
        {isAdding ? (
          <form action={addOngoingSeries} className="grid gap-3 border-b border-hairline px-5 py-4 md:grid-cols-3">
            <input type="hidden" name="fiscalYearId" value={fiscalYearId} />
            ...
            <SoftButton type="submit" variant="primary" className="rounded-none" disabled={isDemo}>Add Series</SoftButton>
          </form>
        ) : null}
        {ongoingSeries.map((item) => (
          ...
          >
            <summary className="grid cursor-pointer list-none items-center gap-4 text-sm md:grid-cols-[1fr_0.6fr_0.9fr_0.5fr_auto] [&::-webkit-details-marker]:hidden">
            ...
            <form action={updateOngoingSeries} className="mt-3 grid gap-3 border-t border-hairline pt-3 md:grid-cols-2">
              ...
              <div className="flex gap-2">
                <SoftButton type="submit" variant="primary" className="rounded-none" disabled={isDemo}>Save Series</SoftButton>
                <SoftButton
