...
import { LIKELIHOOD_RUBRIC, SCORE_RUBRIC, type ScoreRubricBand, STAGE_OPTIONS } from "../coproduction-model";
import type { CoproductionOpportunity } from "../coproduction-types";
import { RubricCard, RubricToggle } from "./rubric-card";
...
/** One field for each score dimension: the number that feeds the grade, the sentence that justifies it, and the standard behind the scale. */
function ScoreField({
  idPrefix,
  dimension,
  label,
  ...
  const panelId = `${idPrefix}-${dimension}-rubric`;
    ...
    <div className="grid gap-2 rounded-md bg-panel-warm p-3">
      <div className="flex items-center justify-between gap-3">
        <span className="flex min-w-0 items-center gap-2">
          ...
          <RubricToggle isOpen={isRubricOpen} controls={panelId} label={label} onToggle={() => setIsRubricOpen((open) => !open)} />
        </span>
      ...
      />
      {isRubricOpen ? <RubricCard id={panelId} question={question} bands={bands} /> : null}
...
export function CoproductionForm({ fiscalYearId, opportunity, onSaved, onDeleted }: CoproductionFormProps) {
  ...
  const [isLikelihoodRubricOpen, setIsLikelihoodRubricOpen] = useState(false);
  const idPrefix = opportunity ? `edit-${opportunity.id}` : "add-coproduction";
  const likelihoodPanelId = `${idPrefix}-likelihood-rubric`;
        ...
        <div className="grid gap-3 md:grid-cols-2">
          ...
          <div className="grid gap-1.5 md:col-span-2">
            <span className="flex min-w-0 items-center gap-2 text-xs font-semibold text-muted">
              <label htmlFor={`${idPrefix}-likelihood-rationale`}>Likelihood rationale</label>
              <RubricToggle
                isOpen={isLikelihoodRubricOpen}
                controls={likelihoodPanelId}
                label="Likelihood"
                onToggle={() => setIsLikelihoodRubricOpen((open) => !open)}
              />
            </span>
            {isLikelihoodRubricOpen ? (
