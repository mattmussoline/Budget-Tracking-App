  ...
  return (
    <div className="grid gap-4">
      {quarters.map((quarter) => (
        <SoftSurface
          key={quarter.quarter}
          className={`overflow-hidden ${quarter.isCurrentQuarter ? "bg-amber-50 outline outline-4 outline-amber-300" : "bg-gray-100"}`}
        >
          <details open={quarter.isCurrentQuarter} className="group">
            <summary className="flex cursor-pointer list-none items-center justify-between gap-4 p-4 marker:hidden md:px-5 md:py-4">
              <span>
                {quarter.isCurrentQuarter ? (
                  <span className="block text-xs font-extrabold uppercase tracking-wide text-amber-700">Current quarter</span>
                ) : null}
                <span className="block font-display text-xl font-extrabold tracking-tight">Quarter {quarter.quarter}</span>
              </span>
              <span className="flex items-center gap-3">
                <span className="rounded-md bg-white px-4 py-2 text-sm font-extrabold text-muted">
            ...
            <div className="grid items-start gap-3 px-4 pb-4 md:px-5 md:pb-5 lg:grid-cols-3">
              {quarter.months.map((month) => {
                ...
                return (
                  <div
                    key={month.index}
                    className={`min-h-28 rounded-lg p-3.5 ${isCurrentMonth ? "bg-amber-100 outline outline-4 outline-amber-400" : "bg-white"}`}
                  >
                    ...
                    <div className="grid gap-2.5">
                      {month.payments.length === 0 ? (
                        ...
                        month.payments.map((payment) => {
                          ...
                          return (
                            <div key={`${payment.licenseId}-${payment.fiscalMonth}`} className={`rounded-lg p-2.5 ${tileClass}`}>
                              <div className="mb-1.5 flex items-start justify-between gap-3">
                                <p className="text-sm font-extrabold leading-tight">{payment.title}</p>
                              ...
                              <div className="mt-1.5 flex items-center justify-between gap-2">
                                ...
                                {payment.isProrated ? (
                                  <span className="rounded-md bg-white/70 px-2 py-1 text-xs font-extrabold">prorated</span>
                                ) : null}
                              </div>
                            </div>
                          );
