    ...
    <div className="grid gap-6">
      {quarters.map((quarter) => (
        <details
          ...
          className={`group min-w-0 rounded-lg shadow-none soft-raised p-5 md:p-6 ${quarter.isCurrentQuarter ? "bg-guild-gold-soft outline outline-4 outline-amber-300" : "bg-panel-warm"}`}
        >
          <summary className="mb-5 flex cursor-pointer list-none items-start justify-between gap-4 [&::-webkit-details-marker]:hidden">
            <div className="min-w-0">
              {quarter.isCurrentQuarter ? (
                <p className="text-xs font-semibold uppercase tracking-wide text-guild-gold-ink">Current quarter</p>
              ) : null}
              <h2 className="font-display text-xl tracking-tight">Quarter {quarter.quarter}</h2>
              <p className="mt-2 w-fit rounded-md bg-white px-3 py-1.5 text-xs font-semibold text-muted">
                {formatCurrency(quarter.months.reduce((total, month) => total + month.totalCents, 0))}
              </p>
            </div>
            <span className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-white text-foreground shadow-sm">
          ...
          <div className="grid gap-4 lg:grid-cols-3">
            {quarter.months.map((month) => {
              ...
              <div
                key={month.index}
                className={`min-h-48 rounded-lg p-4 ${isCurrentMonth ? "bg-guild-gold-soft outline outline-4 outline-amber-400" : "bg-white"}`}
              >
                <div className="mb-4 flex items-center justify-between gap-3">
                  <div>
                    <h3 className="font-display text-lg tracking-tight">{month.label}</h3>
                    {isCurrentMonth ? <span className="text-xs font-semibold uppercase tracking-wide text-guild-gold-ink">Current month</span> : null}
                  </div>
                  <span className="text-sm font-semibold text-muted">{formatCurrency(month.totalCents)}</span>
                </div>
                    ...
                    month.payments.map((payment) => {
                      ...
                      return (
                        <div key={paymentKey} className={`overflow-hidden rounded-lg ${tileClass}`}>
                          <button
                            type="button"
                            className="flex w-full items-center justify-between gap-3 p-3 text-left transition hover:bg-white/25 focus:outline-none focus-visible:ring-4 focus-visible:ring-formed-blue"
                            aria-label={`${isExpanded ? "Collapse" : "Expand"} ${payment.title} payment details`}
                            aria-expanded={isExpanded}
                            onClick={() => togglePayment(paymentKey)}
                          >
                            <span className="min-w-0 text-sm font-semibold leading-snug">{payment.title}</span>
                            <span className="flex shrink-0 items-center gap-2">
                              <span className="text-sm font-semibold">{formatCurrency(payment.amountCents)}</span>
                              <ChevronDown
                                aria-hidden="true"
                            ...
                            <div className="border-t border-white/40 px-3 pb-3 pt-2">
                              <div className="mb-2 flex flex-wrap items-center gap-2">
                                {payment.isFirstPayment ? (
                                  <span className="rounded-md bg-white px-2 py-1 text-[10px] font-semibold uppercase tracking-wide text-foreground">
                                    first
                                  </span>
                                ) : null}
                                {payment.isProrated ? (
                              ...
                              <span className="mt-2 inline-flex rounded-md bg-white/80 px-2 py-1 text-[10px] font-semibold uppercase tracking-wide text-foreground">
                                {budgetSourceLabel}
                              </span>
                              <button
                                type="button"
                                className="mt-3 inline-flex items-center gap-1.5 rounded-md bg-white/85 px-2.5 py-1.5 text-xs font-semibold text-foreground transition hover:bg-white focus:outline-none focus-visible:ring-4 focus-visible:ring-formed-blue"
